...
def distance_cim(lat_start,long_start,lat_end, long_end):
    grad2rad  = ((2 * 3.14) / (360))
    return 6378 * acos(
        sin(grad2rad*lat_start) * sin(grad2rad*lat_end) + cos(grad2rad*lat_start) * cos(lat_end*grad2rad) * cos(grad2rad*(long_end - long_start)))
...
#legge il prezzo del carburante per ogni stazione
fuel_price = np.genfromtxt("data/input_fuel_price.csv", delimiter=",")
fuel_price = fuel_price[:n_stations]

creator.create("FitnessMax", base.Fitness, weights=(-1.0, -1.0,)) #minimizza entrambi gli obiettivi con -1 e -1
creator.create("Individual", list, fitness=creator.FitnessMax) #crea l'oggetto l'individuo che ha come attributo la fitness
...
toolbox.register("population", tools.initRepeat, list, toolbox.individual)#creo shortcut a initRepeat che come sappiamo
...
#a questa funzinone gli passo l'individuo (array di n_cars elementi)
...
def main(seed=None):
    ...
    logbook.record(gen=0, evals=len(invalid_ind), **record) #serve per creare un log di tutti i parametri piu importanti della generazione 0
    ...
    for gen in range(1, NGEN): #effettua NGEN volte le seguenti operazioni, fa queste cose per ogni generazione
        # Vary the population
        offspring = tools.selTournamentDCD(pop, len(pop))#effettua una selezione di tutti gli individui http://gpbib.cs.ucl.ac.uk/gecco2005/docs/p257.pdf
        offspring = [toolbox.clone(ind) for ind in offspring] #clona questi individui quindi fa una riproduzione (non cambia offspring)
        ...
        for ind1, ind2 in zip(offspring[::2], offspring[1::2]): #divide gli individui in due liste separate
            if random.random() <= CXPB: #se il valore di un numero casuale è minore di 0,9 probabilità
                ...
                toolbox.mate(ind1, ind2)#a partire da due genitori ind1 e ind2 mi produce due figli che sovrascrivono ind1 e ind2
        ...
        print (gen,",",record["min"][0],",",record["min"][1],file=f) #scrive la generazione nel file con i valori minimi delle f obiettivo
    f.close()
...
if __name__ == "__main__":
    ...
    best_ind1 = tools.selBest(pop, 1)[0] #seleziona l'individuo che ottimizza la prima f obiettivo
    best_ind2 = tools.selWorst(pop, 1)[0]#seleziona l'individuo che ottimizza la seconda f obiettivo

    f = open('data/out_pareto.csv', 'w') #stampa nel file out_pareto i valori delle fitness di tutti gli individui
                                        #della popolazione finale
    for ind in pop:
    ...
    f = open('data/out_closest_path.csv', 'w') #stampa nel file out_closest_path la posizione dell'auto e la posizione della stazione
                                            #con minore distanza per ogni CS
    for i in range(0,len(best_ind1)):
        print (lat_ecars[i],",",long_ecars[i],",",lat_stations[best_ind1[i]],",",long_stations[best_ind1[i]],file=f)
    f.close()
    ...
    for i in range(0, len(best_ind2)):
        print (lat_ecars[i], ",", long_ecars[i], ",", lat_stations[best_ind2[i]], ",", long_stations[best_ind2[i]],file=f)
    f.close()
    ...
    for i in range(0, n_stations):
        print (stations_closest[i],",",stations_var[i],file=f)



    p_min = np.min(fuel_price)
    p_max = np.max(fuel_price)
    DELTA = abs((p_min - p_max) / 2)

    delta_stations = abs(stations_var-stations_closest)
    max_delta_station = np.max(delta_stations)


    delta_price = -((stations_var - stations_closest) * DELTA) /  max_delta_station



    f = open('data/out_fuel_price.csv','w')
    for i in range(0,len(delta_price)):
        print(delta_price[i],file=f)
    f.close()

    d_max = zeros(n_cars)
    probabilita1 = zeros(n_stations)
    probabilita2 = zeros(n_stations)


    for i in range(0, n_cars):
        d_max[i] = np.max(m_dist[i])

    for i in range(0, n_stations):
        if stations_closest[i] !=0:
            prob = 0
            nprob = 0
            for j in range(0, n_cars):
                if (best_ind1[j] == i):
                    prob = prob + (d_max[j] - m_dist[j][i]) / d_max[j]
                else:
                    nprob = nprob + (d_max[j] - m_dist[j][i]) / d_max[j]

            if stations_closest[i] == n_cars:
                probabilita1[i] = prob / stations_closest[i]
            else:
                probabilita1[i] = prob / stations_closest[i] * (1 - nprob / (n_cars - stations_closest[i]))



    print ("probabilita di trovare auto in coda in base alla distanza dalle stazioni")
    print (probabilita1)
    ...
    for i in range(0, n_stations):
        if stations_var[i] != 0:
            ...
            if stations_closest[i] == n_cars:
                probabilita2[i] = prob / stations_var[i]
            else:
                probabilita2[i] = prob / stations_var[i] * (1 - nprob / (n_cars - stations_var[i]))

            probabilita2[i] = probabilita2[i] + (1 - ((DELTA + delta_price[i]) / (2 * DELTA)))
            probabilita2[i] = probabilita2[i] / 2
    print ("probabilita di trovare auto in coda in base all'equipartizione del traffico e alla variazione di prezzo")
    print (probabilita2)

    f = open('data/out_probability.csv', 'a')
    for i in range(0, len(probabilita1)):
        print(probabilita1[i],",",stations_closest[i],",",probabilita2[i],",",stations_var[i],file=f)
    f.close()
