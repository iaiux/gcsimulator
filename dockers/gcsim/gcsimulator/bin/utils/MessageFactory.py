...
class MessageFactory:
    ...
    def optCriteria(cls):
        ...
        Args:
            actual_time: Simulation Time.
        """
        opt_criteria = Configuration.parameters["opt_criteria"]
        protocol_version = Configuration.parameters["protocol"]
        if protocol_version == "1.0":
            mex = Message(to=cls.basejid + "/actormanager")
            message = "OPTIMIZATION_CRITERIA " + str(opt_criteria)
            mex.body = message
            return mex
        else:
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "OPTIMIZATION_CRITERIA", "type": "' + str(opt_criteria) + '"}}'
            mex.body = message
            mex.metadata = "0"
            return mex
    ...
    def energyCost(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            url = 'localhost'
            mex = Message(to=cls.basejid + "/actormanager")
            message = "ENERGY_COST [0] " + "http://" + str(url) + "/" + str(
                Configuration.mydir) + "/" + str(device.profile) + " " + str(time)
            mex.body = message
            return mex
        else:
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "ENERGY_COST",id: "[99]","profile" : "' + web_url + '/' + str(
                device.profile) + '"}}'
            mex.body = message
            mex.metadata = '0'
            return mex
    ...
    def energyCostProducer(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            url = 'localhost'
            mex = Message(to=cls.basejid + "/actormanager")
            message = "ENERGY_COST [99]:[" + str(device.house) + "]:[" + str(device.device.id) + "] " + "http://" + str(
                url) + "/Results/" + str(mydir) + "/" + str(
                device.energycost) + " " + str(time)
            mex.body = message
            return mex
        else:
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "ENERGY_COST","id": "[99]:[' + str(device.house) + ']:[' + str(
                device.device.id) + ']","profile" : "' + web_url + "/" + str(
    ...
    def energyMix(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            mex = Message(to=cls.basejid + "/actormanager")
            message = "ENERGY_MIX " + str(web_url) + "/" + str(mydir) + "/" + str(device.profile) + " " + str(time)
            mex.body = message
            return mex
        else:
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "ENERGY_MIX",id: "[99]","profile" : "' + web_url + "/" + str(
    ...
    def neighborhood(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "CREATE_ENERGY_GROUP","id": "[99]", "powerpeak" : " ' + str(
                device.peakload) + ' "}}'
            mex.body = message
            mex.metadata = '0'
            return mex
    ...
    def house(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "CREATE_ENERGY_GROUP", "id" : "[99]:[' + str(
                device.id) + ']", "powerpeak" : " ' + str(device.peakload) + ' ", "numcp" : " ' + str(
                device.numcp) + ' ", "lat" : " ' + str(device.lat) + ' ", "long" : " ' + str(device.long) + ' "}}'
            mex.body = message
            mex.metadata = '0'
            return mex
    ...
    def heatercooler(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            message = "HC [" + str(device.house) + "]:[" + str(device.device.id) + "] 0 " + "http://" + str(
                web_url) + "/" + "/" + str(mydir) + "/" + str(device.profile) + " " + str(time)
            mex.body = message
            return (mex)
        else:
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "HC","id" : "[99]:[' + str(device.house) + ']:[' + str(
                device.device.id) + ']","profile" : "' + web_url + "/" + str(mydir) + '/' + str(device.profile) + '"}}'
            mex.body = message
            mex.metadata = time
            return mex
    ...
    def background(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            message = "BG  [" + str(device.house) + "]:[" + str(device.device.id) + "] 0 " + "http://" + str(
                url) + "/" + str(mydir) + "/" + str(device.profile) + " " + str(time)
            mex.body = message
            return mex
        else:

            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "BACKGROUND_LOAD","id" : "[99]:[' + str(device.house) + ']:[' + str(
                device.device.id) + ']","profile" : "' + web_url + "/" + str(
                mydir) + '/' + str(device.profile) + '"}}'
            mex.body = message
            mex.metadata = time
    ...
    def booking_request(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            message = '{"message" : {"subject" : "EV" , "id" : "[99]:[' + str(
                device.device.id) + ']", "soc_at_arrival": " ' + str(
                device.Soc_at_arrival) + ' " , "departure_time" : " ' + str(
                device.actual_departure_time) + ' " , "arrival_time" : " ' + str(
                device.actual_arrival_time) + ' " ,"max_charge_power" : " ' + str(
    ...
    def create_ev(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "CREATE_EV" , "id" : "[99]:[' + str(
    ...
    def create_Battery(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "CREATE_BATTERY" , "id" : "[99]:[' + str(
                device.house) + ']:[' + str(device.device.id) + ']", "capacity" : " ' + str(
                device.device.capacity) + ' " , "max_ch_pow_ac" : " ' + str(
                device.device.max_ch_pow_ac) + ' " , "max_ch_pow_cc" : " ' + str(
                device.device.max_ch_pow_cc) + ' " , "max_all_en" : " ' + str(
                device.device.max_all_en) + ' " , "min_all_en" : " ' + str(
    ...
    def create_producer(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "CREATE_PRODUCER","type" : "PV","id" : "[99]:[' + str(
    ...
    def create_load(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = ' {"message" :  {"subject" : "LOAD", "id" : "[99]:[' + str(device.house) + ']:[' + str(
                device.device.id) + ']", "sequence" : "1", "est" : " ' + str(
                device.est) + ' ", "lst" : " ' + str(
                device.lst) + ' ","profile" : "' + web_url + "/" + str(
                mydir) + '/' + str(device.profile) + '"}} '
            mex.body = message
            mex.metadata = time
            return mex
    ...
    def update_producer(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            mex = Message(to=cls.basejid + "/actormanager")
            message = "PREDICTION_UPDATE [" + str(device.house) + "]:[" + str(device.device.id) + "]  " + str(
                web_url) + "/" + str(mydir) + "/" + str(device.profile) + " " + str(time)
            mex.body = message
            return mex
        else:
            mex = Message(to=cls.basejid + "/" + cls.jid)
            message = '{"message" : {"subject" : "PREDICTION_UPDATE","type" : "PV","id" : "[99]:[' + str(
                device.house) + ']:[' + str(
                device.device.id) + ']","profile" : "' + web_url + "/" + str(
                mydir) + '/' + str(device.profile) + '"}}'
            mex.body = message
    ...
    def delete_load(cls, device, time, protocol_version):
        ...
        if protocol_version == "1.0":
            ...
            message = '{ "message":  {"subject": "DELETE_LOAD", "id": "[99]:[' + str(device.house) + ']:[' + str(
